...
namespace AllHW
{
    internal class HomeWorkFive
    {
        ...
        public class ProblemSeventeen //Again I am using c#'s built in LinkedList<T> as then it won't give me errors for psuedocode.
        {
            public static void SortedAdd(LinkedList<int> List, int index, int Value)
            //Takes in the list, the index you want to add it at, and the value of the new item.
            {
                LinkedListNode<int> ListCurrent = List.First;
                if (index == 0)
                {
                    ListCurrent = List.First; //If they give the 0th index it just grabs the first item and sets that as ListCurrent.
                }
                else
                {
                    for (int i = 0; i < index; i++)
                    {
                        ListCurrent = ListCurrent.Next; //Otherwise, ListCurrent ends up being the index after iterating and 'nexting' itself.
                    }
                }
                List.AddAfter(List.Find(ListCurrent.Value - 1), Value); //Lastly it adds the new node where it should be.
            }

            public static void SortedDelete(LinkedList<int> List, int index) 
            //We take in a list, and the index to be deleted.
            {
                LinkedListNode<int> ListCurrent = List.First;
                if (index == 0)
                {
                ...
                List.Remove(ListCurrent); //Lastly it removes the item at that index.
            }
        }
    }
}